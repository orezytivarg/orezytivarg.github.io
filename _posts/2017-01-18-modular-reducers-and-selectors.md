...
그 아티클에서 Jack은 프로젝트 구조화에 대해 3가지 법칙을 마련했다:
...
## 모듈이 마운트 컨트롤을 제어하게 하라

Jack의 기사에서 말하기를:

> todos 모듈에게 자신의 state 원자에 대한 마운트를 제어할 수 있도록 하여 이 문제를 해결할 수 있다
...
또다른 선택지는 조금 복제하여 살아가는 것이다. 나는 현재 이 접근법을 사용하고 있지만, Jack의 방법으로 스위칭할 것을 고려중이다.
...
실제 세계에서는 [이전에 언급](http://randycoulman.com/blog/2016/02/16/using-ramda-with-redux/)했던 [Ramda](http://ramdajs.com/)를 이용한다. 조금 더 깨끗하게 보일 것이다:
...
내 리듀서와 그 테스트에서는 개별 셀렉터를 임포트하고 다른 곳에서는 디폴트를 임포트해서 글로벌 셀렉터를 임포트한다. 지금은 이게 혼동되는지 잘 모르겠지만, 앞으로는 따라가기 어려울 수 있다.

## 결론

말했다시피 아직 완전히 만족스러운 해결책은 찾지 못했다. 나는 리듀서가 어디에 마운트되어 있는지에 대해 복제를 이용하여 살아가고 있다.

모듈이 마운트되는 곳을 제어하는 Jack의 접근법으로 전환을 진지하게 고려중이다.
